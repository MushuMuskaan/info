...
export const SearchPage: React.FC = () => {
  const { isUser, isInfoWriter, isAdmin, userProfile, loading: authLoading } = useAuth();
  ...
  const loadSearchData = (): (() => void) => {
    ...
    const articlesQuery = query(
      ...
      limit(100) // Limit for better search performance
    );

    const unsubscribe = onSnapshot(articlesQuery, (snapshot) => {
      try {
        console.log("SearchPage: Received snapshot with", snapshot.size, "documents");
        ...
        console.log("SearchPage: Processed", publishedArticles.length, "articles");
