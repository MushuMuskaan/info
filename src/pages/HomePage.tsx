...
import { ArticleCardSkeleton, FeaturedArticleSkeleton, CategorySkeleton } from "../components/LoadingSkeleton";
...
export const HomePage: React.FC = () => {
  ...
  // Simple inline processing without complex functions
  const processArticleData = (articles: Article[]) => {
    // Simple featured articles - just take first 6 with cover images
  ...
  const loadHomePageData = (): (() => void) => {
    ...
    // Create optimized real-time listener for articles with pagination
    ...
    const unsubscribe = onSnapshot(articlesQuery, (snapshot) => {
      try {
        console.log("HomePage: Received snapshot with", snapshot.size, "documents");
        ...
        console.log("HomePage: Processed", publishedArticles.length, "articles");

        // Memoized expensive operations
        const processedData = processArticleData(publishedArticles);

        setHomeData({
          articles: publishedArticles,
          featuredArticles: processedData.featuredArticles,
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-700 mb-2">No Articles Available</h2>
