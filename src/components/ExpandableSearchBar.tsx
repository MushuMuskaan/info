...
export const ExpandableSearchBar: React.FC<ExpandableSearchBarProps> = ({
  ...
  // Load articles on mount with real-time updates
  useEffect(() => {
    if (!userProfile) return;

    // Create optimized real-time listener for recent articles only
    const articlesQuery = query(
      collection(firestore, "articles"),
      where("status", "==", "published"),
      orderBy("publishedAt", "desc"),
      limit(50) // Limit to recent articles for search suggestions
    );
