...
// Role-aware article functions that respect visibility rules
export const getVisibleArticles = async (
  ...
  if (options.authorId || options.status || userRole !== "admin") {
    if (options.limit) {
...
export const getVisiblePublishedArticles = async (userRole: string = "user"): Promise<Article[]> => {
  return getVisibleArticles(userRole, { status: "published" });
};

export const getVisibleUserArticles = async (authorId: string, userRole: string = "user"): Promise<Article[]> => {
  return getVisibleArticles(userRole, { authorId });
};
