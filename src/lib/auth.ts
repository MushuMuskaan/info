...
export const removeInfoWriterStatus = async (uid: string, removedByUid?: string) => {
  const userRef = doc(firestore, "users", uid);

  // Update user role and add removal tracking
  await updateDoc(userRef, {
    role: "user",
    updatedAt: new Date(),
    previousRoles: [uid], // Track that this user was previously an infowriter
    privilegesRemovedAt: new Date(),
    privilegesRemovedBy: removedByUid || "system",
  });
  ...
  const updatePromises = articlesSnapshot.docs.map(doc => {
    const data = doc.data();
    // Only hide published articles, leave drafts as they are
    if (data.status === "published") {
      return updateDoc(doc.ref, {
        status: "hidden",
        hiddenAt: new Date(),
        hiddenBy: removedByUid || "system",
        hiddenReason: "InfoWriter privileges removed",
        originalStatus: "published" // Track original status for potential restoration
      });
    }
    return Promise.resolve(); // Return resolved promise for non-published articles
  });
